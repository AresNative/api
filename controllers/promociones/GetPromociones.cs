...
{
    public partial class Promociones : BaseController
    {
        public Promociones(IConfiguration configuration) : base(configuration) { }
        public class PromocionesRequest
        {
            public List<BusquedaParams> Filtros { get; set; } = new();
        }

        [HttpPost("api/v2/select/promociones")]
        public async Task<IActionResult> ObtenerPromocionesRequest(
            [FromBody] PromocionesRequest request,
            ...
            [FromQuery] int pageSize = 10)
        {
            ...
            var baseQuery = @"
            FROM [LOCAL_TC032391E].[dbo].[Website_promotion]";
